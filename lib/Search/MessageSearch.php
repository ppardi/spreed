...
class MessageSearch implements IProvider, IFilteringProvider {
	...
	protected function commentToSearchResultEntry(Room $room, IUser $user, IComment $comment, ISearchQuery $query): SearchResultEntry {
		...
		$messageStr = $this->formatMessageText($message->getMessage(), $message->getMessageParameters(), $query->getTerm());
	...
	protected function performFederatedSearch(IUser $user, ISearchQuery $query, string $title, Room $room, Participant $participant): SearchResult {
		...
		$result = array_map(
			fn (array $message): SearchResultEntry => $this->federatedMessageToSearchResultEntry($room, $user, $message, $query),
			$messages,
		);
	 ...
	 */
	protected function federatedMessageToSearchResultEntry(Room $room, IUser $user, array $message, ISearchQuery $query): SearchResultEntry {
		...
		return $this->createSearchResultEntry(
			...
			$this->formatMessageText($message['message'], $message['messageParameters'], $query->getTerm()),
			(string)$message['id'],
			null,
			$message['actorType'],
			$message['actorId'],
	...
	/**
	 * The message with its parameters written out, shortened at the start when the search term is further in
	 *
	 * @param array<string, TalkRichObjectParameter> $messageParameters
	 */
	protected function formatMessageText(string $message, array $messageParameters, string $term): string {
		$search = $replace = [];
		...
		$matchPosition = mb_stripos($message, $term);
		if ($matchPosition > 30 && mb_strlen($message) > 40) {
			// Mostlikely the result is not visible from the beginning,
			// so we cut of the message a bit.
			$message = '…' . mb_substr($message, $matchPosition - 10);
		}
