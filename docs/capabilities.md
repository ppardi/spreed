...
* `federated-read-status` - Whether the host reports the common read marker for a federated conversation. Clients should only show the read status in a federated conversation when both the local server and the host server list the feature.
